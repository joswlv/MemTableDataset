...
public class DefaultKeyedValues implements KeyedValues4Default, Serializable {

    private static final long serialVersionUID = -201204142349L;
    private List<String> rowKeys;
    private List<String> columnKeys;

    private List<MemKeyedValues> rows;
    private boolean sortRowKeys;
    public DefaultKeyedValues() {
        this(false);
    }

    public DefaultKeyedValues(boolean sortRowKeys) {
        this.rowKeys = new java.util.ArrayList<String>();
        this.columnKeys = new java.util.ArrayList<String>();
        this.rows = new java.util.ArrayList<MemKeyedValues>();
    ...
    public Number getValue(int row, int column) {
        Number result = null;
        MemKeyedValues rowData = this.rows.get(row);
        if (rowData != null) {
    ...
    public Number getValue(String rowKey, String columnKey) {
        ...
        if (row >= 0) {
            MemKeyedValues rowData
                = (MemKeyedValues) this.rows.get(row);
    ...
    public void removeValue(String rowKey, String columnKey) {
        ...
        int rowIndex = getRowIndex(rowKey);
        MemKeyedValues row = (MemKeyedValues) this.rows.get(rowIndex);
        ...
        for (int item = 0, itemCount = this.rows.size(); item < itemCount;
             item++) {
            row = (MemKeyedValues) this.rows.get(item);
    ...
    public void removeColumn(String columnKey) {
        ...
        Iterator<MemKeyedValues> iterator = this.rows.iterator();
        while (iterator.hasNext()) {
            MemKeyedValues rowData = iterator.next();
            int index = rowData.getIndex(columnKey);
            if (index >= 0) {
    ...
    public boolean equals(Object o) {
        ...
        if (!(o instanceof KeyedValues4Default)) {
            return false;
        }
        KeyedValues4Default kv2D = (KeyedValues4Default) o;
