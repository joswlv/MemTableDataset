...
public class MemKeyedValues implements KeyedValues, Serializable {
    ...
    public synchronized void insertValue(int position, String key, Number value) {
    ...
    public void sortByKeys(SortOrder order) {
        ...
        final Dataset4Sort[] data = new Dataset4Sort[size];

        for (int i = 0; i < size; i++) {
            data[i] = new Dataset4Sort(this.keys.get(i), this.values.get(i));
        }
        Arrays.sort(data, new KeyedValueComparator(KeyedValueComparatorType.BY_KEY, order));
        clear();

        for (int i = 0; i < data.length; i++) {
            final Dataset4Sort value = data[i];
